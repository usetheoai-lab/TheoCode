...
- **`package-lock.json` still pins `@theokit/agents@10.1.0` while the manifests ask for `^11.0.0`.** Recorded rather than fixed, because the fix is a decision this entry cannot make for whoever owns the install. The upgrade below moved the manifests and the lockfile was never regenerated, so the two disagree: `npm ls` reports `invalid: "^11.0.0" from packages/agent` on all four workspaces. It has been invisible because the working `node_modules` was installed by **pnpm**, which reads `pnpm-lock.yaml` and resolves 11.0.0 correctly — so anyone running `pnpm install` sees a healthy tree, and only `npm ci` (which all five CI jobs run) would get 10.1.0. Reproduced by running `npm install` in this checkout: the tree silently degraded to 10.1.0 and `typecheck` began failing on `onError`, a field that only exists in 11.0.0. Regenerating the npm lockfile does NOT fix it — `npm install --package-lock-only` re-pins 10.1.0. Two lockfiles for one repository is the underlying condition; the entry that follows claims the upgrade shipped, and against `npm` it did not.
