...
**115 files, 882 cases** (measured 2026-09-03 — `npm test`'s own count, which is the only one that is right: a glob over `*.test.ts` finds 114 because one suite is a `.test.mjs` under `tools/`, and counting `it(` finds 823 because `it.each` expands). The following were left out by an explicit decision —
