...
**117 files, 900 cases** (measured 2026-09-03 — `npm test`'s own count, which is the only one that is right. Two naive substitutes both undercount: a glob over `*.test.ts` finds 106, because 8 suites are `.test.tsx` and 3 are `.test.mjs`; and `grep -c 'it('` across all three extensions finds 866, because `it.each` expands into one case per row). The following were left out by an explicit decision —
